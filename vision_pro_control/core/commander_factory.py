...
def create_robot_commander(
    ...
    Examples:
        >>> # 自动选择（推荐）
        >>> commander = create_robot_commander('192.168.8.10')

        >>> # 使用 joint 控制
        >>> commander = create_robot_commander('192.168.8.10', backend='joint')
    """
...
def create_robot_commander_from_config(config: dict):
    """
    从配置字典创建控制器
    ...
    return create_robot_commander(robot_ip=robot_ip, backend=backend)
